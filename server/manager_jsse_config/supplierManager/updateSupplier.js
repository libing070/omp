...
(function(param, header) {
	...
	try {
		if(!supplierNumber || !supplierID || !supplierName|| !shortName || !contactPhone || !claimPhone) {
			logger.error("参数缺失");
			errorResponse(-1);
			return;
		}
		...
		var shortNameCount = SupplierDomain.checkShortName(sqlAdapter,shortName);
		if(shortNameCount > 0) {
		...
		SupplierDomain.updateSupplier(sqlAdapter,supplierObj);
