 ...
 */
app
	.controller('listPaymentDetailCtrl', function($scope, $http, $timeout, $location, serverUrl, openCity, userAuth) {
		/**
		 * 初始化订单状态
		 */
		listSupplier(); //加载供应商列表
		listPaymentDetail(null); //初始化列表
		 ...
		 * 初始化科目状态
		 */
		$scope.subjectStatus = ['保费', '批改缴费', '批改退费', '核保失败退费', '投保失败退费', '退保', '车船税'];
		...
		function getInfo() {
			var queryParam = {
				...
				startDateStart: $scope.dates.search_createTimeStart ? $scope.dates.search_createTimeStart.format('YYYY-MM-DD HH:mm') : "",
				startDateEnd: $scope.dates.search_createTimeEnd ? $scope.dates.search_createTimeEnd.format('YYYY-MM-DD HH:mm') : "",
				resourceType: $scope.search_resourceType || '',
				goodsOrderNo: $scope.search_goodsOrderNo || '',
		...
		function setArr() {
			...
			param = {};
			if (params.startDateStart && params.startDateStart != null) {
				param.key = "起始日期:";
				param.value = params.startDateStart;
				result.push(param);
				$scope.isReal = true;
			}

			param = {};
			if (params.startDateEnd && params.startDateEnd != null) {
				param.key = "终止日期:";
				param.value = params.startDateEnd;
				result.push(param);
				$scope.isReal = true;
			}
			...
			if (params.resourceType && params.resourceType != null) {
				param.key = "用户:";
				param.value = params.resourceType == 1 ? '用户' : '险企';
				result.push(param);
				$scope.isReal = true;
			}

			param = {};
			if (params.goodsOrderNo && params.goodsOrderNo != null) {
				param.key = "订单编号:";
				param.value = params.goodsOrderNo;
				result.push(param);
				$scope.isReal = true;
			}
		...
		function exportReport(obj) {
			...
			$http({
				method: 'post',
				data: obj,
				url: serverUrl + 'exportManager/exportPaymentDetail'
			}).then(function successCallback(response) {
				if (response.data.errorCode == 0) {
					$scope.popShow = false;
		...
		function listPaymentDetail(obj) {
			if (!obj) {
				obj = {};
				obj['ticket'] = $location.$$search.ticket;
				obj['domain'] = $location.$$search.domain;
			...
			$http({
				method: 'post',
				data: obj,
				url: serverUrl + 'feeManager/queryPaymentDetail'
			}).then(function successCallback(response) {
				if (response.data.errorCode == 0) {
					$scope.paymentDetailList = response.data.data.paymentDetailList;
					$scope.total = response.data.data.totalCount;
					$scope.currentPage = obj.pageNumber;
					console.log($scope.paymentDetailList);
					$scope.DoCtrlPagingAct = function(text, page, pageSize, total) {
						...
						data.sortFieldName = "payDate";
						console.log(JSON.stringify(data));
						listPaymentDetail(data);
					};
				} else {
					userAuth.isLogin(response.data);
					$scope.title = "提示";
